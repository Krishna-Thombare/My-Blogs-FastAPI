...
from schemas import (
    PostResponse, UserCreate, 
    UserUpdate, UserPublic, 
    UserPrivate, Token, 
    PaginatedPostsResponse, 
    ChangePasswordRequest, ForgetPasswordRequest, ResetPasswordRequest)
...
async def login_for_access_token(
    ...
    # Note: OAuthPasswordRequestForm uses "username" field, but we treat it as email
    result = await db.execute(
        select(models.User).where(
    ...
    # Don't reveal which one failed (best security practice)
...
async def delete_user(
    ...
    # Delete the profile picture image after deleting the user
    if old_filename:
...
async def upload_profile_picture(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid image file. Please upload a valid imagee (JPEG, PNG, GIF, WebP)."
        ) from err
